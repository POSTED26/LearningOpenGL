...
// settings
unsigned int SCR_WIDTH = 800;		// should be const
...
int main()
{
	...
	// setting the bit depth of the color channels and the refresh rate of the monitor
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "POSTED", monitor, NULL);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ImGui::Checkbox("light rotation", &lightRotate);
		ImGui::End();
		...
		if (lightRotate)
		{
			lightPos = glm::vec3(2.0f * cos(glfwGetTime()), 1.0f, 2.0f * sin(glfwGetTime()));
